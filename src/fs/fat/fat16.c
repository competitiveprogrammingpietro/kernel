...
static void fat16_init_private(struct disk *disk, struct fat_private *private)
{
    ...
    private
        ->cluster_read_stream = disk_stream_new(disk->id);
    private
        ->fat_read_stream = disk_stream_new(disk->id);
    private
        ->directory_stream = disk_stream_new(disk->id);
}
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory)
{
    ...
    struct disk_stream *stream = fat_private->directory_stream;
    if (disk_stream_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != PEACHOS_OK)
    {
        res = -EIO;
        goto out;
    }
