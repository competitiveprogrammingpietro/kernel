...
// align the given address to the next page boundary
void *paging_align_address_next(void *ptr)
{
...
// align the given address to the previous page boundary
void *paging_align_address_previous(void *ptr)
{
	if (PAGE_IS_ALIGNED(ptr))
	{
		return ptr;
	...
	uint32_t address = (uint32_t)ptr;
	return (void *)(address - (address % PAGING_PAGE_SIZE));
}
